...
if [ -z "$ANSIBLE_PLAYBOOK_TAGS" ]; then
	ansible-playbook -i CloudFormation/CodeBuild/hosts "$ANSIBLE_PLAYBOOK_PATH"
else
	ansible-playbook -i CloudFormation/CodeBuild/hosts "$ANSIBLE_PLAYBOOK_PATH" -t "$ANSIBLE_PLAYBOOK_TAGS"
fi
